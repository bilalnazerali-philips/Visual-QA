...
# Guard against an accidental broad deletion when this script is run in CI.
...
try {
    ...
    Write-Host "==> Restore cross-platform solution"
    ...
    Copy-Item -LiteralPath "visual-tests/PatientInfo/wpf/actual.png" -Destination (Join-Path $packageRoot "example/example-failing-actual.png")

    $validationOutput = Join-Path $packageRoot "example-result"
    $cliPath = Join-Path $packageRoot "VisualQa.Cli.exe"
    Write-Host "==> Validate published package with its bundled example"
    & $cliPath compare-images `
        --reference (Join-Path $packageRoot "example/approved-reference.png") `
        --actual (Join-Path $packageRoot "example/example-failing-actual.png") `
        --output $validationOutput `
        --config (Join-Path $packageRoot "visualqa.json")
    if ($LASTEXITCODE -notin @(0, 1)) {
        throw "Published CLI validation failed with exit code $LASTEXITCODE."
    }
